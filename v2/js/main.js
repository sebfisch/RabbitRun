...
const game = new Game();
// Debug-Zugriff, z. B. in der Browser-Konsole
window.rabbitrun = game;
...
// Wisch nach unten = Sturzflug. Der Wisch beginnt zwangsläufig mit einem
// pointerdown (das einen Sprung auslöst) – die Sprunglogik erstattet einen
// dabei verbrauchten Doppelsprung wieder zurück.
let swipe = null;
...
window.addEventListener('pointerdown', (e) => {
  ...
  swipe = { y: e.clientY, t: performance.now() };
  game.onPress();
});
window.addEventListener('pointermove', (e) => {
  if (!swipe) return;
  if (e.clientY - swipe.y > 26 && performance.now() - swipe.t < 300) {
    swipe = null;
    game.onDive();
  }
});
...
window.addEventListener('keydown', (e) => {
  ...
  if (JUMP_KEYS.includes(e.code)) {
    ...
    initAudio();
    game.onPress();
  } else if (DIVE_KEYS.includes(e.code)) {
    e.preventDefault();
    game.onDive();
  } else if (e.code === 'KeyM') {
    toggleMute();
